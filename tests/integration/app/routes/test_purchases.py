...
class Testpurchases:
    def test_create_purchase_with_submission(
        self, auth_user_client, submission, purchase_data
    ):
    ...
    def test_create_purchase_frozen_user(
        self, user, auth_user_client, auth_mod_client, purchase_data
    ):
    ...
    def test_update_purchase(self, auth_admin_client, purchase):
        ...
        response = auth_admin_client.patch(
            f"/purchases/{purchase.id}", json=update_purchase_data
        )
        assert response.status_code == 200
