...
class TestRewards:
    ...
    def test_upload_reward_image(
        self, auth_admin_client, reward, reward_image_file_path
    ):
        with open(reward_image_file_path, "rb") as image_file:
            response = auth_admin_client.post(
                f"/rewards/{reward.id}/image", data={"image": image_file}
            )
        assert response.status_code == 200

    def test_upload_reward_image_inadequate_access(
        self, auth_mod_client, reward, reward_image_file_path
    ):
        with open(reward_image_file_path, "rb") as image_file:
    ...
    def test_upload_reward_image_invalid_data(self, auth_admin_client, reward):
        response = auth_admin_client.post(
            f"/rewards/{reward.id}/image", data={"image": "data"}
        )
    ...
    def test_update_reward(self, auth_admin_client, reward):
        ...
        response = auth_admin_client.patch(
            f"/rewards/{reward.id}", json=update_reward_data
        )
        assert response.status_code == 200
