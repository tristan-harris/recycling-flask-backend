...
class TestSubmissions:
    ...
    def test_create_submission_frozen_user(self, user, auth_user_client, auth_mod_client, submission_post_data):
    ...
    def test_update_submission(self, auth_mod_client, submission):
        ...
        response = auth_mod_client.patch(f"/submissions/{submission.id}", json=update_submission_data)
        assert response.status_code == 200

    def test_update_submission_inadequate_access(self, auth_user_client, submission):
        update_submission_data = {"status": "confirmed"}
        response = auth_user_client.patch(f"/submissions/{submission.id}", json=update_submission_data)
        assert response.status_code == 403

    def test_update_submission_invalid_data(self, auth_admin_client, submission):
        update_submission_data = {"data": "data"}
        response = auth_admin_client.patch(f"/submissions/{submission.id}", json=update_submission_data)
