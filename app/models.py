...
from sqlalchemy import ForeignKey, Integer, Float, String, Boolean, Enum, Date, DECIMAL, TIMESTAMP, JSON
...
class Motivation(db.Model):
    ...
    id: Mapped[int] = \
        mapped_column(Integer, primary_key = True, nullable = False, autoincrement = True)
    user_id: Mapped[int] = \
        mapped_column(Integer, ForeignKey("users.id"), unique = True, nullable = False)
    motivation: Mapped[str] = mapped_column(String(2000), nullable = False)
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP, nullable = False)

    def to_dict(self) -> dict:
        return {"user_id": self.user_id, "motivation": self.motivation,
...
class Bin(db.Model):
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "latitude": float(self.latitude), "longitude": float(self.longitude),
                "whitelist": self.whitelist, "name": self.name, "description": self.description,
                "created_at": int(self.created_at.timestamp())}
...
class AllowedRecyclable(db.Model):
    ...
    id: Mapped[int] = \
        mapped_column(Integer, primary_key = True, nullable = False, autoincrement = True)
    bin_id: Mapped[int] = \
        mapped_column(Integer, ForeignKey("bins.id"), primary_key = True, nullable = False)
    recyclable_id: Mapped[int] = \
        mapped_column(Integer, ForeignKey("recyclables.id"), primary_key = True, nullable = False)
...
class Recyclable(db.Model):
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "type": self.type, "points_value": self.points_value,
...
class Submission(db.Model):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key = True, nullable = False, autoincrement = True)
    recyclable_id: Mapped[int] = mapped_column(Integer, ForeignKey("recyclables.id"), nullable = False)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable = False)
    bin_id: Mapped[int] = mapped_column(Integer, ForeignKey("bins.id"), nullable = False)
    latitude: Mapped[float] = mapped_column(DECIMAL(9, 6), nullable = False)
    longitude: Mapped[float] = mapped_column(DECIMAL(9, 6), nullable = False)
    status: Mapped[SubmissionStatus] \
        = mapped_column(Enum(SubmissionStatus), nullable = False, default = SubmissionStatus.not_confirmed)
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP, nullable = False)

    def to_dict(self) -> dict:
        return {"id": self.id, "recyclable_id": self.recyclable_id, "user_id": self.user_id,
                "bin_id": self.bin_id, "latitude": float(self.latitude), "longitude": float(self.longitude),
                 "status": self.status.value, "created_at": int(self.created_at.timestamp())}
...
class Reward(db.Model):
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "title": self.title, "description": self.description,
                "price": self.price, "created_at": int(self.created_at.timestamp())}
...
class Purchase(db.Model):
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "user_id": self.user_id, "reward_id": self.reward_id,
                "quantity": self.quantity, "created_at": int(self.created_at.timestamp())}
...
class UserActionLog(db.Model):
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "user_id": self.user_id, "action_type": self.action_type.value,
                "resource_id": self.resource_id, "resource_table": self.resource_table,
                "data_before": self.data_before, "data_after": self.data_after,
