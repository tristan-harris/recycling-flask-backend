...
def create_submission():
    ...
    if not db_controller.is_owner_or_admin(current_user.id, User, request_data["user_id"]):
        return RouteErrors.UNAUTHORISED_ACCESS.value

    if db_controller.is_frozen(current_user.id):
        return {"error": "Your account is frozen and you cannot make new submissions"}, 403
    ...
    if bin.whitelist:
        whitelisted_recyclables = db_controller.get_bin_whitelist(request_data["bin_id"])
        whitelisted = False
    ...
    distance = haversine((request_data["latitude"], request_data["longitude"]),
        (bin.latitude, bin.longitude), unit=Unit.METERS)
...
def upload_submission_image(id:int):
    ...
    image_path = get_image_upload_path(id, current_app.config["UPLOADS_DIRECTORY"], Constants.SUBMISSION_IMAGES_DIRECTORY)
...
def update_submission(id:int):
    ...
    validation_result = validate_data(request_data, SubmissionUpdateSchema, partial=True)
    if not validation_result.valid:
        return {"error": validation_result.error_message, "message": validation_result.info}, 400
    request_data = validation_result.data
